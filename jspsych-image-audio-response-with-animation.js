...
jsPsych.plugins["image-audio-response-with-animation"] = (function() {
    ...
    jsPsych.pluginAPI.registerPreload(
        "image-audio-response-with-animation",
        "stimulus",
        "image"
    );
    ...
    plugin.info = {
        ...
        parameters: {
            ...
            show_mic_label: {
                type: jsPsych.plugins.parameterType.BOOL,
                pretty_name: "Show mic name ",
                default: false,
                description: "Display name of the microphone or not.",
            },
    ...
    plugin.trial = function(display_element, trial) {
        if (typeof trial.stimulus === "undefined") {
            console.error(
                'Required parameter "stimulus" missing in image-audio-response'
            );
        }
        ...
        detect_mic(function(has_mic) {
            if (!has_mic) {
                ...
                var html =
                    '<img src="' +
                    trial.stimulus +
                    '" id="jspsych-image-audio-response-stimulus" style="';
                ...
                if (trial.show_mic_label) {
                    html += "<h4>" + mic_label + "</h4>";
                }
                ...
                html +=
                    '<div id="jspsych-image-audio-response-recording-container">' +
                    trial.recording_light_off +
                    "</div>";

                // add audio element container with hidden audio element
                html +=
                    '<div id="jspsych-image-audio-response-audio-container"><audio id="jspsych-image-audio-response-audio" controls style="visibility:hidden;"></audio></div>';

                // add button element with hidden buttons
                html +=
                    '<div id="jspsych-image-audio-response-buttons"><button id="jspsych-image-audio-response-okay" class="jspsych-audio-response-button jspsych-btn" style="display: inline-block; margin:' +
                    trial.margin_vertical +
                    " " +
                    trial.margin_horizontal +
                    '; visibility:hidden;">Okay</button><button id="jspsych-image-audio-response-rerecord" class="jspsych-audio-response-button jspsych-btn" style="display: inline-block; margin:' +
                    trial.margin_vertical +
                    " " +
                    trial.margin_horizontal +
                    '; visibility:hidden;">Rerecord</button></div>';

                function start_trial() {
                    display_element.innerHTML = html;
                    document
                        .querySelector("#jspsych-image-audio-response-okay")
                        .addEventListener("click", end_trial);
                    document
                        .querySelector("#jspsych-image-audio-response-rerecord")
                        .addEventListener("click", start_recording);
                    // Add visual indicators to let people know we're recording
                    document.querySelector(
                        "#jspsych-image-audio-response-recording-container"
                    ).innerHTML = trial.recording_light;
                    // trial start time
                    ...
                    if (trial.stimulus_duration !== null) {
                        jsPsych.pluginAPI.setTimeout(function() {
                            display_element.querySelector(
                                "#jspsych-image-audio-response-stimulus"
                ...
                function process_audio(stream) {
                    ...
                    var micName = audioSourceInfo.label;
                    if (trial.wait_for_mic_approval) {
                        if (start_time === null) {
                            start_trial();
                        } else {
                    ...
                    recorder.ondataavailable = function(e) {
                        ...
                        if (recorder.wrapUp) {
                            if (typeof trial.postprocessing !== "undefined") {
                                trial.postprocessing(chunks).then(function(processedData) {
                ...
                function showPlaybackTools(data) {
                    // Audio Player
                    let playerDiv = display_element.querySelector(
                        "#jspsych-image-audio-response-audio-container"
                    );
                    let url;
                    ...
                    let player = playerDiv.querySelector(
                        "#jspsych-image-audio-response-audio"
                    );
                    player.src = url;
                    player.style.visibility = "visible";
                    // Okay/rerecord buttons
                    let buttonDiv = document.querySelector(
                        "#jspsych-image-audio-response-buttons"
                    );
                    let okay = buttonDiv.querySelector(
                        "#jspsych-image-audio-response-okay"
                    );
                    let rerecord = buttonDiv.querySelector(
                        "#jspsych-image-audio-response-rerecord"
                    );
                ...
                function onRecordingFinish(data) {
                    ...
                    if (light !== null) light.innerHTML = trial.recording_light_off;
                    // measure rt
                    ...
                    if (trial.response_ends_trial) {
                        end_trial();
                    } else if (trial.allow_playback) {
                        // only allow playback if response doesn't end trial
